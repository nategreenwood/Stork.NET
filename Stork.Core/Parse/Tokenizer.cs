        ...
        public Token NextToken()
        {
            ...
            else if (char.IsDigit(Convert.ToChar(GetInput.Peek()))) // Numeric type
            {
                ...
                if (GetInput.Peek() == '.')
                {
                    ...
                    {
                        throw new ParseException("Floating-point numbers must have at least one digit after the decimal.");
                    }
                }
                else
                {
                ...
                while(GetInput.Peek() != -1 && GetInput.Peek() != '\n' && GetInput.Peek() != '\"')
                {
                    ...
                    if(c == '\\') // THis might be an escape character
                    {
                        if(GetInput.Peek() != -1)
                        {
                            ...
                            {
                                throw new ParseException("Unrecognized escape sequence: " + Convert.ToChar(GetInput.Peek()), GetInput.GetOffset());
                            }
                        }
                        else
                        {
                            throw new ParseException("Unexpected end of file", GetInput.GetOffset());
                        }
                    }
                    else
                    {
            ...
            else if (_operatorCharacters.Contains(Convert.ToChar(GetInput.Peek()))) // Operator
            {
                ...
                if(!_operatorTypes.ContainsKey(buffer.ToString().Trim()))
                    throw new ParseException("Unrecognized operator: " + buffer.ToString(), offset);
            ...
            {
                throw new ParseException("Unrecognized character; " + Convert.ToChar(GetInput.Peek()));
            }
